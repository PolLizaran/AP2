...
using namespace std;
using participants = pair<string, int>;
...
void read_actions(map<string, int>& server, map<string, int>& out_server, string s){
  if (s == "LOGIN"){
    string player1;
    cin >> player1;
    if (not server.count(player1)) server.insert({player1, 1200});
...
void write_ranking(map<string, int>& server,map<string, int>& out_server){
  ...
  for(auto p : server) ranking.push_back({p.first, p.second});
  for(auto j : out_server) ranking.push_back({j.first, j.second});
  sort(ranking.begin(), ranking.end(), conditions);
  for(participants p : ranking) cout << p.first << ' ' << p.second << endl;
